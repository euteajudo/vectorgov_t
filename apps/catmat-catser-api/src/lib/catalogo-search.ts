...
/**
 * Busca trigram (FTS5 tokenize='trigram') — match por **substring/parcial** rápido
 * (≈ `LIKE '%x%'` acelerado por GIN no Postgres). Cada palavra (≥ 3 chars) vira um
 * termo de substring, unidos por OR — assim "procedim" acha "procedimento" e
 * termos parciais/incompletos casam. (Não é correção de digitação por edit-distance,
 * que seria o `similarity()` do pg_trgm — só substring.)
 */
async function queryTrgmCatalogo(
  ...
  const where: string[] = ["catalogo_trgm MATCH ?"];
  ...
  const sql = `
    SELECT
      ...
      c.grupo AS grupo,
      c.classe AS classe,
      bm25(catalogo_trgm) AS rank
    FROM catalogo_trgm f
    JOIN catalogo_itens c ON c.id = f.catalogo_id
    WHERE ${where.join(" AND ")}
    ORDER BY rank ASC
    LIMIT ?
...
export async function buscarCatalogoHibrido(
  ...
  const [vecHits, ftsRows, trgmRows] = await Promise.all([
  ...
  const fused = Array.from(ids)
    .map((id) => {
      ...
      const item = row ? itemFromRow(row) : itemFromMeta(vecById.get(id)!.metadata);
...
/** Busca lexical (grep) — só D1 FTS5/BM25 (unicode61). */
